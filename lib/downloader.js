 ...
 *   junk payloads; only bodies carrying the PNG signature (and the
 *   45-byte structural minimum) are accepted. Tiny transparent ocean
 *   tiles pass, mislabeled JSON/HTML never does
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (!isPngBody(data)) {
            ...
            const err = new Error(
              `discarded ${data.length}-byte body (not a PNG)`,
            );
            err.permanent = true;
            throw err;
          }
