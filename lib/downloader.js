 ...
 *   tiny payloads; anything under the configured `minTileBytes` is
 *   discarded and counted as a failure, never inserted (SPEC Addendum 2)
 * - validates the media type of successful responses: the exact
 *   Content-Type must be `image/png`. JSON or HTML bodies (often rate
 *   limits) are dropped and trigger the escalating backoff throttle
 *   (SPEC Addendum 6)
...
/**
 * Default minimum tile body size (SPEC Addendum 2, OpenSeaMap baseline).
 * Providers can override via `minTileBytes` (Open Waters uses 300,
 * SPEC Addendum 6).
 */
 ...
 * @param {number} [options.minTileBytes] - Minimum accepted tile body
 *   size (default 500, SPEC Addendum 2; Open Waters uses 300, SPEC
 *   Addendum 6)
...
function createDownloader(options) {
  ...
  const minTileBytes = Math.max(1, options.minTileBytes ?? MIN_TILE_BYTES);
   ...
   * undersized bodies) throw immediately; wrong-media-type bodies and
   * rate limits (429/503) wait out the escalating penalty; other
   * transient errors (5xx, network faults, timeouts) retry with
    ...
    for (;;) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          // with HTTP 200 + JSON/HTML bodies. Only an exact
          // `image/png` media type is a raster tile.
          const mediaType = mediaTypeOf(res);
          if (mediaType !== "image/png") {
            await discardBody(res);
            escalatePenalty();
            const err = new Error(
              `discarded body with Content-Type ${mediaType || "(missing)"} (expected image/png)`,
            );
            err.rateLimited = true;
            throw err;
          }
          const data = Buffer.from(await res.arrayBuffer());
          if (data.length < minTileBytes) {
            // Rate-limit placeholders answer 200 OK with tiny bodies
            const err = new Error(
              `discarded ${data.length}-byte tile body (< ${minTileBytes})`,
            );
            err.permanent = true;
            throw err;
          }
