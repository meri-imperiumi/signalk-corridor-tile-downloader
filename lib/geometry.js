...
/**
 * Geographic bounding box of the route padded by the corridor margin
 * (margin converted to degrees; 1 degree of latitude = 60 NM), clamped
 * to the Web Mercator valid range and MBTiles lon/lat order.
 *
...
/**
 * Every tile from z0 through `toZoom` intersecting a geographic
 * rectangle — the low-zoom "overview pyramid" each corridor job
 ...
 * spans a handful of tiles, and the rectangle is what the chart's
 * bounds advertise anyway. Bounded by construction — the widest
 * possible corridor bounds at z0-z7 are a few hundred tiles.
 *
 ...
 */
function overviewTiles(bounds, toZoom) {
 ...
 * metadata bounds (padded by the widest configured margin).
 *
 ...
 * @returns {{tiles: Array<{z: number, x: number, y: number, yTms: number}>, bounds: [number, number, number, number]|null}}
 */
function corridorTiles(coordinates, options) {
  ...
  if (valid.length === 0) return { tiles: [], bounds: null };
  ...
  return { tiles, bounds: boundsWithMargin(valid, widestMargin) };
}
