...
/**
 * Inverse Web-Mercator helpers for the extent derivation below.
 * `normX`/`normY` are normalized tile coordinates over the whole
 * pyramid at a zoom: x/2^z grows west→east, the y here is top-origin
 * (0 at the projection's north edge).
 */
function normXToLon(norm) {
  return norm * 360 - 180;
}

function normYToLat(norm) {
  return (Math.atan(Math.sinh(Math.PI * (1 - 2 * norm))) * 180) / Math.PI;
}

/**
 * Aggregate query deriving the envelope of every stored tile, in
 * normalized pyramid units (so tiles at different zooms compare).
 * `tile_row` is TMS (bottom origin): the NORTH edge of a tile is
 * `(y+1)/2^z` from the top, its SOUTH edge `y/2^z`.
 */
const EXTENT_SQL = `
  SELECT
    MIN(CAST(tile_column AS REAL) / (1 << zoom_level)) AS w,
    MAX((CAST(tile_column AS REAL) + 1) / (1 << zoom_level)) AS e,
    1 - MAX((CAST(tile_row AS REAL) + 1) / (1 << zoom_level)) AS n,
    1 - MIN(CAST(tile_row AS REAL) / (1 << zoom_level)) AS s,
...
/**
 * Derives the geographic extent ACTUALLY covered by stored tiles —
 * the authority on what the cache can serve. Unlike the `bounds`
 * metadata row (which only ever describes the corridor of the latest
 * job while tiles accumulate across jobs), this can never shrink
 * below the data on disk. Null when the file holds no tiles.
 *
 * @param {import("node:sqlite").DatabaseSync} db
 * @returns {{bounds: [number, number, number, number], minzoom: number, maxzoom: number}|null}
 */
function extentFromTilesDb(db) {
  const row = db.prepare(EXTENT_SQL).get();
  if (row == null || row.w == null) return null;
  const round = (v) => Math.round(v * 1e6) / 1e6;
  return {
    bounds: [
      round(Math.max(-180, normXToLon(row.w))),
      round(Math.max(-85.051129, normYToLat(row.s))),
      round(Math.min(180, normXToLon(row.e))),
      round(Math.min(85.051129, normYToLat(row.n))),
    ],
    minzoom: row.minzoom,
    maxzoom: row.maxzoom,
  };
...
/**
 * Read-only variant of the extent derivation for files not held open
 * by a store (the resource provider and style transform probe idle
 * caches without disturbing write handles). Null when the file cannot
 * be opened or holds no tiles.
 *
 * @param {string} filePath
 * @returns {{bounds: [number, number, number, number], minzoom: number, maxzoom: number}|null}
 */
function extentFromTilesFile(filePath) {
  let db;
  try {
...
/**
 * Run a PASSIVE WAL checkpoint after this many successful inserts.
 * Committed tiles live in the `-wal` sidecar until a checkpoint moves
 * them into the main database; charts-provider-simple's startup
 * housekeeping deletes sidecars, and a crash before the close-time
 * checkpoint loses them. Capping the un-checkpointed inserts to this
 * many limits a restart to re-fetching at most this many tiles per
 * store. PASSIVE never waits for readers (no added contention).
 */
...
class MbTilesStore {
  ...
  constructor(filePath, options = {}) {
    ...
    this._insertsSinceCheckpoint = 0;
    this._open();
  }
  ...
  insertTile(z, x, yTms, data) {
    ...
    if (result.changes > 0) {
      // Periodic PASSIVE checkpoint: committed tiles live in the
      // `-wal` sidecar until a checkpoint moves them into the main
      // database. charts-provider-simple's startup housekeeping
      // deletes sidecars, and a crash before the close-time
      // checkpoint loses them — so flush the WAL into the main db
      // every CHECKPOINT_INTERVAL inserts. PASSIVE never waits for
      // readers (no added contention). Best-effort: a failed
      // checkpoint (e.g. a concurrent reader) just defers to the next.
      this._insertsSinceCheckpoint += 1;
      if (this._insertsSinceCheckpoint >= CHECKPOINT_INTERVAL) {
        this._insertsSinceCheckpoint = 0;
        try {
          this.db.exec("PRAGMA wal_checkpoint(PASSIVE);");
        } catch {
          // A concurrent reader can fail a PASSIVE checkpoint; the
          // close-time checkpoint and the next interval cover it.
        }
      }
    }
  ...
  /**
   * Derives the cache's true coverage from the tiles table (see
   * `extentFromTilesDb`).
   *
   * @returns {{bounds: [number, number, number, number], minzoom: number, maxzoom: number}|null}
   */
  extentFromTiles() {
    this._healIfSidecarWedged();
    return this._withIoRetry(() => extentFromTilesDb(this.db));
  }
  ...
  vacuum() {
    ...
    this._withIoRetry(() => {
      ...
      // only a checkpoint truncates the main file, which is the one
      // whose size the status panel reports. Best-effort: a concurrent
      // reader can block TRUNCATE, and the close-time checkpoint still
      // reclaims the space once it goes away.
      try {
        this.db.exec("PRAGMA wal_checkpoint(TRUNCATE);");
      } catch {
        // Deferred to the close-time checkpoint
      }
    });
  }

  /**
   * Deletes every cached tile and reclaims the disk space (DELETE +
   * VACUUM — VACUUM alone only compacts free pages, it never removes
   * live rows). Must not run while a download job holds write cycles;
   * the REST layer enforces that. The file survives as an empty, still
   * loadable chart: `format` stays (it must match the configured tile
   * format across refills), `bounds` collapses back to the empty-cache
   * placeholder, and the cleared corridor's zoom rows go away.
   */
  clear() {
    this._healIfSidecarWedged();
    this._withIoRetry(() => {
      this.db.exec("DELETE FROM tiles;");
      this.db.exec("VACUUM;");
      // In WAL mode VACUUM's compacted pages land in the -wal sidecar;
      // only a checkpoint truncates the main file, which is the one
      // whose size the status panel and the consumer see. Best-effort:
      // a concurrent reader can block TRUNCATE, and the close-time
      // checkpoint still reclaims the space once it goes away.
      try {
        this.db.exec("PRAGMA wal_checkpoint(TRUNCATE);");
      } catch {
        // Deferred to the close-time checkpoint
    ...
    this.setMetadata("bounds", "0,0,0,0");
    this._deleteMetadata.run("minzoom");
    this._deleteMetadata.run("maxzoom");
  }
